...
class Undochakra extends Table
{
    ...
    */
    ...
    {    
        ...
        $nbColor = 8;
        if(self::getGameStateValue( 'moduleA') == 2)
        {
            $nbColor = 9;
        }
        ...
        self::setGameStateInitialValue( 'withUndo', 1 );
        ...
        self::initStat( 'player', 'YinYang', 0 );
        self::initStat( 'player', 'Objectives', 0 ); 
        
        //Module D: objectives
        if(self::getGameStateValue( 'moduleD') == 2)
        {
            //debug
            $nb = count($players) == 2?2:3;
            while(self::getUniqueValueFromDB( "SELECT count(*) FROM objective") < $nb)
            {
                $type = bga_rand(1,8);
                if(self::getUniqueValueFromDB( "SELECT count(*) FROM objective where type = ".$type) == 0)
                {
                    self::DbQuery("INSERT INTO objective (type) VALUES (".$type.")");
                }
            }
        }
    ...
    function isHarmonized($player_id, $row, $withTemp = false)
    {
        if($row <= 1 || $row >= 9) return false;
        $color = $this->colors[$row-1];
        return self::getUniqueValueFromDB( "SELECT count(*) FROM energy where location='".$player_id."' and row = ".$row." and color = '".$color."'") == 3
        || ($withTemp && self::getUniqueValueFromDB( "SELECT count(*) FROM energy where location='".$player_id."' and row = ".$row) == 3 && self::getUniqueValueFromDB( "SELECT count(*) FROM energy where location='".$player_id."' and row = ".$row." and color = 'white'") >= 1); // NOI18N
    }
    
    function isHarmonizedTable($table, $row, $withTemp = false)
    {
        if($row <= 1 || $row >= 9) return false;
        ...
        {
            $white = $white || $energy['color'] == "white";
            $other = $other || $energy['color'] != $color;
        }
        return !$other || ($white && $withTemp);
    }
    ...
    */
    protected function getAllDatas()
    {
        ...
        $sql = "SELECT player_id id, yinyang, player_score score, player_no position, if(purple>0,1,0) purple, if(darkblue>0,1,0) darkblue, if(blue>0,1,0) blue, if(green>0,1,0) green, if(yellow>0,1,0) yellow, if(orange>0,1,0) orange, if(red>0,1,0) red FROM player ";
        ...
        $sql = "SELECT id, type, token1, token2 FROM objective";
        $result['objectives'] = self::getObjectListFromDB( $sql );  
...
//////////// 
    ...
    function actUndo()
    {
        ...
        {
            //prevents database update error
        }        
    ...
    function actTake( $energyIds, $row )
    {
        ...
        else
        {
            ...
            if(self::getUniqueValueFromDB( "SELECT count(*) FROM energy where location ='maya' and (color='black' or color='white') and col=".$column)>0 && !in_array('black', $colors) && !in_array('white', $colors))
            {
                throw new feException( "You must take at least one black or one white");
            }
            ...
            foreach($energies as $id => $energy)
            {
                if($energy['color'] != 'black' && $energy['color'] != 'white' && self::getUniqueValueFromDB( "SELECT count(*) FROM energy where location ='".$player_id."' and color = '".$energy['color']."'")>=3)// NOI18N
                {
...
////////////
    ...
    */
    ...
    function getNextTableState($table, $channel, $am, $energy, $diff)
    {
        ...
        for($i=2;$i<=8;$i++)
        {
            $harmonized[$i] = $this->isHarmonizedTable($table, $i, true);
        }
        $harmonized[1] = false;
        $harmonized[9] = false;
        
        if(!$this->isHarmonizedTable($table, $energy['row'], false) && !in_array($energy['id'],$alreadyMoved) && $energy['row'] != 9)
        {
            $possible = true;
            $harmo = 0;
            $rowTested = 0;
            ...
            for($d=1;$d<=abs($diff) && $possible;$d++)
            {
                ...
                else
                {
                    ...
                    if( ($energy['color'] == 'black' || $energy['color'] == 'white') && $rowTested == 9)
                    {
                        continue;
                    }
                    $possible = false;
    ...
    { 
        ...
        foreach($colors as $color => $colorval)
        {
            if($color == 'black' || $color == 'white' || self::getUniqueValueFromDB( "SELECT count(*) FROM energy where location =".$player_id." and color='".$color."'" )<3)// NOI18N
            {
                ...
                if(!$harmonized[$energy['row']] && !in_array($energy['id'],$alreadyMoved) && $energy['row']!=9)
                {
                    ...
                    for($p=$min;$p<$max;$p++)
                    {
                        ...
                        for($d=1;$d<=abs($diff) && $possible;$d++)
                        {
                            ...
                            else if($this->isHarmonized($player_id, $rowTested, true))
                            {
                                $harmo++;
                                $d--;
                            }
...
////////////
    ...
    function stCheckFinish()
    {
        ...
        //check for yinyang effect
        for($row=2;$row<=8;$row++)
        {
            $white = self::getObjectFromDb( "select * from energy where location = '".$player_id."' and row = ".$row." and color = 'white' LIMIT 1");
            $black = self::getObjectFromDb( "select * from energy where location = '".$player_id."' and row = ".$row." and color = 'black' LIMIT 1");
            
            if($white != null && $black != null)
            {
                self::DbQuery( "delete from energy where id = ".$white['id']);
                self::DbQuery( "delete from energy where id = ".$black['id']);
                self::DbQuery( "update player set yinyang = yinyang+1 where player_id = ".$player_id);
                $player['yinyang'] = intval($player['yinyang']) + 1;
                
                self::notifyAllPlayers( "yinyang", clienttranslate('${player_name} earns half a Yin-Yang symbol'), array(
                    'player_name' => $player['player_name'],
                    'player_id' => $player_id,
                    'nb' => $player['yinyang'],
                    'energy1' => $white['id'],
        ...
        $sql = "SELECT id, type, token1, token2 FROM objective";
        $objectives = self::getObjectListFromDB( $sql );
        foreach( $objectives as $obj_id => $obj )
        {
            if($obj['token1'] != $player_id && $obj['token2'] != $player_id
                && ($obj['token1'] == null || $obj['token2'] == null))
            {
                $win = false;
                switch($obj['type'])
                {
                    case 1:
                        $win = $player['purple'] != "0" && $player['darkblue'] != "0"  && $player['blue'] != "0"  && $player['green'] != "0"  && $player['yellow'] != "0"  && $player['orange'] != "0"  && $player['red'] != "0" ;
                        break;
                        
                    case 2:
                        $win = intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 9 and color = 'black' and location='".self::getActivePlayerId()."'")) >= 3;
                        break;
                   
                    case 3:
                        $win = intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 2 and color = 'purple' and location='".self::getActivePlayerId()."'")) >= 1
                        && intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 3 and color = 'darkblue' and location='".self::getActivePlayerId()."'")) >= 1
                        && intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 4 and color = 'blue' and location='".self::getActivePlayerId()."'")) >= 1
                        && intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 5 and color = 'green' and location='".self::getActivePlayerId()."'")) >= 1
                        && intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 6 and color = 'yellow' and location='".self::getActivePlayerId()."'")) >= 1
                        && intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 7 and color = 'orange' and location='".self::getActivePlayerId()."'")) >= 1
                        && intval(self::getUniqueValueFromDB(  "SELECT count(*) FROM energy where row = 8 and color = 'red' and location='".self::getActivePlayerId()."'")) >= 1;
                        break;
                        
                    case 4:
                        $win = ($this->isHarmonized($player_id, 2) && $this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 4))
                        || ($this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 5))
                        || ($this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 5) && $this->isHarmonized($player_id, 6))
                        || ($this->isHarmonized($player_id, 5) && $this->isHarmonized($player_id, 6) && $this->isHarmonized($player_id, 7))
                        || ($this->isHarmonized($player_id, 6) && $this->isHarmonized($player_id, 7) && $this->isHarmonized($player_id, 8));
                        break;
                        
                    case 5: 
                        $win = ($this->isHarmonized($player_id, 2) && $this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 5))
                        || ($this->isHarmonized($player_id, 2) && $this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 5) && $this->isHarmonized($player_id, 6))
                        || ($this->isHarmonized($player_id, 2) && $this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 6) && $this->isHarmonized($player_id, 7))
                        || ($this->isHarmonized($player_id, 2) && $this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 7) && $this->isHarmonized($player_id, 8))
                        || ($this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 5) && $this->isHarmonized($player_id, 6))
                        || ($this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 6) && $this->isHarmonized($player_id, 7))
                        || ($this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 7) && $this->isHarmonized($player_id, 8))
                        || ($this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 5) && $this->isHarmonized($player_id, 6) && $this->isHarmonized($player_id, 7))
                        || ($this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 5) && $this->isHarmonized($player_id, 7) && $this->isHarmonized($player_id, 8))
                        || ($this->isHarmonized($player_id, 5) && $this->isHarmonized($player_id, 6) && $this->isHarmonized($player_id, 7) && $this->isHarmonized($player_id, 8));
                        
                        break;
                        
                        
                    case 6:
                        $win = $this->isHarmonized($player_id, 2) && $this->isHarmonized($player_id, 8) ;
                        break;
                    case 7:
                        $win = $this->isHarmonized($player_id, 4) && $this->isHarmonized($player_id, 6) ;
                        break;
                    case 8:
                        $win = $this->isHarmonized($player_id, 3) && $this->isHarmonized($player_id, 7) ;
                        break;
                }
                                
                if($win)
                {
                    $nb = $obj['token2'] == null?2:1;
                    self::DbQuery( "update objective set token".$nb." = ".$player_id." where id = ".$obj['id']);
                    self::notifyAllPlayers( "objo", clienttranslate('${player_name} earns ${nb} point(s) with objective : ${objective_type}'), array(
        ...
        if(self::getGameStateValue( 'withUndo') == 1)
        {
            try
            {
            $this->undoSavepoint();
        ...
        {   
            ...
            {                
                ...
                {
                    $table[$color] = array();
                    $table[$color][0] = '<div class="meditation '.$color.'"></div>';
                    if($color != 'black')
                    {
                        $value = self::getUniqueValueFromDB( "SELECT value FROM plenitude where color='".$color."'");
            ...
            $table['black'][0] =  array( "str" => clienttranslate('Alleviated energy'), "args" => array()  );
            ...
            //End of the game            
            $sql = "SELECT player_id id, player_name, player_color, player_score, yinyang FROM player ";
            $players = self::getCollectionFromDb( $sql );
            $harmo_max = 1;
            $player_harmo_max = -1;
            ...
            { 
                ...
                for($row=8;$row>=2;$row--)
                {
                    ...
                    {
                        $value = self::getUniqueValueFromDB( "SELECT value FROM plenitude where color='".$color."'");
                        $table[$color][$playerindex] = $value;
                        $players[$player_id]['player_score'] += $value;
            ...
            if(self::getGameStateValue( 'moduleD') == 2)
            {
                $table['e'] = array( array( "str" => clienttranslate('Objectives bonus'), "args" => array()  ) );
                                
                $playerindex=1;
                foreach( $players as $player_id => $player )
                {
                    $nb1 = intval(self::getUniqueValueFromDB( "SELECT count(*) FROM objective where token1='".$player_id."'"));// NOI18N
                    $nb2 = 2*intval(self::getUniqueValueFromDB( "SELECT count(*) FROM objective where token2='".$player_id."'"));// NOI18N
                    
                    $players[$player_id]['player_score'] += ($nb1+$nb2);
                    $this->setStat(($nb1+$nb2), 'Objectives', $player_id);
                    $table['e'][$playerindex] = ($nb1+$nb2);
                    $playerindex++;
                }
            }
            ...
            if(self::getGameStateValue( 'moduleA') == 2)
            {
                $table['c'] = array( array( "str" => clienttranslate('Yin-Yang bonus'), "args" => array()  ) );
                
                $playerindex=1;
                foreach( $players as $player_id => $player )
                {
                    $nb = intval($player['yinyang']);
                    $nb = 3 * intdiv($nb,2) + $nb%2;
                    $players[$player_id]['player_score'] += $nb;
                    $this->setStat($nb, 'YinYang', $player_id);
                    $table['c'][$playerindex] = $nb;
                    $playerindex++;
                }
            }
             
            
            $table['a'] = array( array( "str" => clienttranslate('Harmonization bonus'), "args" => array()  ) );
            $table['z'] = array( array( "str" => clienttranslate('Plenitude level'), "args" => array()  ) );
            $playerindex=1;
            foreach( $players as $player_id => $player )
            {
                ...
                $table['black'][$playerindex] = $nbBlack;
                $this->setStat(0, 'harmo_points', $player_id);
                $table['a'][$playerindex] = 0;
                ...
                $table['z'][$playerindex] = $players[$player_id]['player_score'];
                
                $playerindex++;
                
                self::notifyAllPlayers( "harmonize", '', array(
                    'player_id' => $player_id,
                    'score' => $players[$player_id]['player_score']
                ) );
            }   
                        
            $this->notifyAllPlayers( "tableWindow", '', array(
                "id" => 'finalScoring',
                "title" => clienttranslate("Final Plenitude levels"),
                "table" => $table,
...
//////////
    ...
    */
    ...
    {
        if( $from_version <= 2103221030 )
        {
            try
            {
                // ! important ! Use DBPREFIX_<table_name> for all tables
                $sql = "CREATE TABLE IF NOT EXISTS DBPREFIX_objective ( `id` int(3) NOT NULL AUTO_INCREMENT, `type` int(3) NOT NULL, `token2` int(10) NULL, `token1` int(10) NULL, PRIMARY KEY (`id`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8";
        ...
        if( $from_version <= 2103221030 )
        {
            ...
            {
                // ! important ! Use DBPREFIX_<table_name> for all tables
                $sql = "ALTER TABLE DBPREFIX_player ADD `yinyang` int(5) NOT NULL DEFAULT '0'";
                self::applyDbUpgradeToAllDB( $sql );
            }
        ...
        if( $from_version <= 2103221422 )
        {
            try
            {
                // ! important ! Use DBPREFIX_<table_name> for all tables
                $sql = "ALTER TABLE zz_savepoint_player ADD `yinyang` int(5) NOT NULL DEFAULT '0'";
                self::applyDbUpgradeToAllDB( $sql );
            }
